...
AYAKA_NODISCARD inline bool is_row_major_contiguous(const ayaka_tensor_view_t* v) {
    int64_t expected = 1;
...
AYAKA_NODISCARD inline bool same_shape(const ayaka_tensor_view_t* a, const ayaka_tensor_view_t* b) {
    if (a->rank != b->rank) {
        return false;
    }
